...
class TribleConv(nn.Module):
    def __init__(self, in_channels, out_channels, dhw):
        ...
        self.trible_conv = nn.Sequential(
            nn.Conv3d(in_channels, out_channels, kernel_size=7, stride=1, padding=3, bias=True),
...
class Down(nn.Module):
    def __init__(self, in_channels, out_channels, dwh):
        ...
        self.encoder = nn.Sequential(
            nn.MaxPool3d(2, 2),
            TribleConv(in_channels, out_channels, dwh)
        )
...
#         self.conv = TribleConv(in_channels, out_channels)
#
...
class Encoder1(nn.Module):
    def __init__(self, in_channels=1, n_classes=1, n_channels=16):
        super().__init__()
        self.in_channels = in_channels  # 1
        self.n_classes = n_classes  # 8
        # n_channels -> out_channels
        self.n_channels = n_channels  # 4
        self.conv = TribleConv(in_channels, n_channels, 224)
...
class Transencoder(nn.Module):
    def __init__(self):
        super().__init__()
        self.transencoder = timm.models.create_model("vit_small_patch16_224", num_classes=1, in_chans=14)
...
class TransNet3d_test(nn.Module):
    def __init__(self, in_channels=1, n_classes=8, n_channels=16):
...
if __name__ == '__main__':
    model = TransNet3d()
    x = torch.randn((1, 1, 224, 224, 224), dtype=torch.float32)  # 默认为 torch.float32
    x = x.to("cuda:1")
